...
| File | What it tests |
|---|---|
| **[ternary_qat_cifar10.py](ternary_qat_cifar10.py)** | Trains a small CNN three ways on CIFAR-10 — `fp32` / **train-time ternary** (BitNet-style straight-through estimator) / **post-hoc ternary** — and measures the gap. The core claim made touchable: *post-hoc ternary collapses; train-time ternary is near-lossless.* The STE code is a teaching tool for "how do you make {−1, 0, +1} learnable?" Runs on a Mac (MPS). |
| **[ternary_vs_int8_reram_noise.py](ternary_vs_int8_reram_noise.py)** | Injects ReRAM conductance noise (sweeps σ) and measures level-flips + cosine corruption on real ternary vs INT8 weights. The chip's core device-risk (R1), made runnable. |
